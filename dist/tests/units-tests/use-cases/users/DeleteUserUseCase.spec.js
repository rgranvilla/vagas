...
// src/tests/units-tests/use-cases/users/DeleteUserUseCase.spec.ts
...
describe("Delete User", () => {
  ...
  let sut;
  let createdUser;
  let createdUserWithoutDeletePermission;
  beforeEach(async () => {
  ...
  it("should able to delete a user", async () => {
    ...
    globalExpect(beforeDeleteUser).toHaveLength(2);
    await sut.execute(createdUser.id);
    const afterDeleteUser = await dbRepository.getUsers();
    globalExpect(afterDeleteUser).toHaveLength(1);
  });
  it("should throw error on delete a user when dont have user with unexistent id", async () => {
    ...
    await globalExpect(() => sut.execute(UnexistentId)).rejects.toBeInstanceOf(
      ResourceNotFoundError
    );
    globalExpect(executeSpy).toBeCalled();
    const afterDeleteUser = await dbRepository.getUsers();
    globalExpect(afterDeleteUser).toHaveLength(2);
  });
});
