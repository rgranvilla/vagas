...
// src/tests/units-tests/use-cases/sessions/AuthenticateUserUseCase.spec.ts
...
var AuthenticateUserUseCase = class {
  ...
  async execute(name, password) {
    ...
    const passwordMatch = await (0, import_bcryptjs.compare)(
      password,
      user.password
    );
    if (!passwordMatch) {
...
describe("Authenticate User", () => {
  ...
  it("should able to authenticate user", async () => {
    ...
    globalExpect(token).toEqual(globalExpect.any(String));
  });
});
